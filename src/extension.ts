...
function findBuildFiles(context: vscode.ExtensionContext): { jsFile: string | null, cssFile: string | null } {
    const fs = require('fs');
    const path = require('path');
    
    try {
        const jsDir = path.join(context.extensionPath, 'frontend', 'build', 'static', 'js');
        const cssDir = path.join(context.extensionPath, 'frontend', 'build', 'static', 'css');
        ...
        if (fs.existsSync(jsDir)) {
            ...
            jsFile = jsFiles.find((file: string) => file.startsWith('main.') && file.endsWith('.js') && !file.endsWith('.map'));
        }
        
        // Find main CSS file (starts with "main." and ends with ".css")
        if (fs.existsSync(cssDir)) {
            const cssFiles = fs.readdirSync(cssDir);
            cssFile = cssFiles.find((file: string) => file.startsWith('main.') && file.endsWith('.css') && !file.endsWith('.map'));
        }
...
function getWebviewContent(panel: vscode.WebviewPanel, context: vscode.ExtensionContext, astResult: string, selectedText: string): string {
    ...
    const cssSrc = panel.webview.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, "frontend", "build", "static", "css", cssFile));
    ...
    <html lang="en">
      <head>
        ...
        <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${panel.webview.cspSource} 'unsafe-inline'; script-src ${panel.webview.cspSource} 'unsafe-inline' 'unsafe-eval'; font-src ${panel.webview.cspSource}; img-src ${panel.webview.cspSource} data:;">
